...
export default function LetterForm() {
  ...
  const handleSendEmail = async () => {
    ...
    const selectedTemplate = templates.find(t => t.id === parseInt(formData.selectedTemplate));
    if (!selectedTemplate) {
      setError('Érvénytelen sablon');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        setError('Nem sikerült elküldeni a levelet');
      }
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button
          ...
          className={`w-full flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white 
            ${isLoading ? 'bg-gray-400' : isSent ? 'bg-green-600' : 'bg-pink-600 hover:bg-pink-700'}
        ...
        >
          {isLoading ? (
            ...
            <>
              <Send className="mr-2 h-4 w-4" /> Előnézet
            </>
          )}
        </button>
      </form>
